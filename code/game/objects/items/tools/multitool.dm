...
/obj/item/multitool/proc/set_multitool_buffer(mob/user, obj/machinery/M)	// Loads a machine into memory, returns TRUE if it does
	if(!ismachinery(M))
		...
		return
	buffer = M
	to_chat(user, span_notice("Вы загрузили [M.declent_ru(ACCUSATIVE)] во внутренний буфер [declent_ru(GENITIVE)]."))
	return TRUE
...
/obj/item/multitool/brass
	...
	ru_names = list(
		NOMINATIVE = "латунный мультитул",
		GENITIVE = "латунного мультитула",
		DATIVE = "латунному мультитулу",
		ACCUSATIVE = "латунный мультитул",
		INSTRUMENTAL = "латунным мультитулом",
		PREPOSITIONAL = "латунном мультитуле"
	)
